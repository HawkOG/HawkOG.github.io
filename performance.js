...
xhr.onreadystatechange = function () {
	if (xhr.readyState === 4) {
		...
		for (a = 0; a < package.length; a++) {
			var newHtml = `<li class="child" style="display:inline-block;width:200px"><div class="card mt-2 p-0 col-3 w-100" id="card"><p class="card-text text-center pt-1 itemTitle"><strong>${package[a].title}</strong></p><img class="card-img-top" src="${package[a].image}"><div class="card-body"><p class="card-text float-left cat"><strong>${package[a].category}</strong></p><p class="card-text float-right"><strong>${Math.floor(Math.random()*15 + 1)},${Math.floor(Math.random()*5 + 1)}${Math.floor(Math.random()*9 + 1)} €</strong></p></li>`;
